...
for i = 1:10
    ...
    [w1(i,:), w2(:,i), b1(i,:), b2(i)] = NeuralNetworkRegressionReLU(subtrainData, subtrainLabels, d1, eta, iter, iw1, iw2, ib1, ib2);
    
    yTrain = ReLU(trainData * w1(i,:) + b1(i,:)) * w2(:,i) + b2(i);
    trainError(i) = mean_squared_error(yTrain, trainLabels);
    
    yTest = ReLU(testData * w1(i,:) + b1(i,:)) * w2(:,i) + b2(i);
...
[w1f, w2f, b1f, b2f] = NeuralNetworkRegressionReLU(trainData, trainLabels, d1, eta, iter, iw1, iw2, ib1, ib2);
figure;
x = 0: 0.001: 1;
y = ReLU(x' * w1f + b1f) * w2f + b2f;
plot(x, y);
hold on;
